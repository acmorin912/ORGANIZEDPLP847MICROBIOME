...
count(as.data.frame(as.matrix(sample_data(
  physeq_object_roots))), Description, BarcodePlate)
...
head(sample_data(physeq_object_roots))

#still getting issues here, may have been done above so I am skipping ahead
contaminants <-
  isContaminant(
    physeq_object_roots,
    method = "prevalence",
    batch = "BarcodePlate",
    neg = "is.neg", ##THIS IS THE ISSUE HERE
    threshold = 0.4)

table(contaminants$contaminant) 

#Cannot do this next bit cuz I cannot get above to work
# remove contaminants ---------------------------------------------------
remove_taxa = function(physeq_object_roots, badTaxa){
  allTaxa = taxa_names(physeq_object_roots)
  myTaxa <- allTaxa[!(allTaxa %in% badTaxa)]
  return(prune_taxa(myTaxa, physeq_object_roots))
}

physeq_filt <-
  remove_taxa(physeq_object_roots, rownames(subset(
    contaminants, contaminant%in%c("TRUE"))))

physeq_filt
#Continuing ahead with "physeq_object_roots"

# remove control samp[les  
physeq_clean <-
  subset_samples(physeq_object_roots, is.neg%in%c("FALSE")) #error here too. 'is.neg' not found
...
ggplot(dat.agr, 
  ...
  geom_bar(stat = "identity")+ #this line changes the Abundance axis 
  scale_fill_manual(values = c("Albifimbria" ="#652926",
                               ...
                               "Setophoma" = "darkgoldenrod2"))+ #for some reason this line is changing the graph to only display like... 4 genera.
  # Remove x axis title
